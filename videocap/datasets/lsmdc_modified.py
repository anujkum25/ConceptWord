...
class DatasetLSMDC():
    ...
    def __init__(self,
        ...
        if self.use_tgif:
            ...
            self.msr_h5 = self.read_msr_hdf5()
            self.crc_h5 = self.read_crc_hdf5()
    ...
    def read_msr_hdf5(self):
        if self.image_feature_net.lower() == 'resnet':
            if self.layer.lower() == 'res5c':
                feature_file = os.path.join(VIDEO_FEATURE_DIR, 'MSR_' + self.image_feature_net.upper()+".hdf5")
                assert_exists(feature_file)
            elif self.layer.lower() == 'pool5':
                feature_file = os.path.join(VIDEO_FEATURE_DIR, "MSR_" + self.image_feature_net.upper()
    ...
    def read_crc_hdf5(self):
        if self.image_feature_net.lower() == 'resnet':
            if self.layer.lower() == 'res5c':
                feature_file = os.path.join(VIDEO_FEATURE_DIR, 'CRC_' + self.image_feature_net.upper()+".hdf5")
                assert_exists(feature_file)
            elif self.layer.lower() == 'pool5':
                feature_file = os.path.join(VIDEO_FEATURE_DIR, "CRC_" + self.image_feature_net.upper()
                                            + "_" + self.layer.lower() + ".hdf5")
                assert_exists(feature_file)
            elif self.layer.lower() == 'fc1000':
                feature_file = os.path.join(VIDEO_FEATURE_DIR, "CRC_" + self.image_feature_net.upper()
    ...
    def read_df_from_csvfile(self):
        ...
        if self.data_type == 'CAP' and self.use_tgif and self.dataset_name == 'train':
            tgif_data_path = os.path.join(DATAFRAME_DIR, 'TGIF.csv')
            assert_exists(tgif_data_path)
            tgif_df = pd.read_csv(tgif_data_path, sep='\t')
            # TODO modify TGIF.csv to have vid_key column
            tgif_df['vid_key'] = list(tgif_df.loc[:, 'key'])
            data_df = data_df.append(tgif_df.loc[:, ['key', 'vid_key', 'description']])

            msr_data_path = os.path.join(DATAFRAME_DIR, 'MSR_CAP_train.csv')
            assert_exists(msr_data_path)
            msr_df = pd.read_csv(msr_data_path, sep='\t')
            data_df = data_df.append(msr_df.loc[:, ['key', 'vid_key', 'description']])

            crc_data_path = os.path.join(DATAFRAME_DIR, 'VAS_CAP_train.csv')
            assert_exists(crc_data_path)
            crc_df = pd.read_csv(crc_data_path, sep='\t')
            data_df = data_df.append(crc_df.loc[:, ['key', 'vid_key', 'description']])

            para_data_path = os.path.join(DATAFRAME_DIR, 'Paraphrase_CAP_train.csv')
            assert_exists(para_data_path)
            para_df = pd.read_csv(para_data_path, sep='\t')
            data_df = data_df.append(para_df.loc[:, ['key', 'vid_key', 'description']])
    ...
    def get_attribute_mask(self, key):
        vid_id = self.data_df.loc[key, 'vid_key']
        return self.attribute_extractor.get_top_attribute(vid_id)[1]
