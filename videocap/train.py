...
def main(argv):
    ...
    with g.as_default():
        ...
        for step in range(train_config.max_steps):
            mean_loss, concept_loss, current_step, time_delta = trainer.run_single_step(
                queue=train_queue, is_train=True)

            if current_step % train_config.steps_per_logging == 0:
                trainer.log_step_message(current_step, mean_loss, concept_loss,
                                         time_delta, steps_in_epoch, is_train=True)

            if current_step % train_config.steps_per_evaluate == 0:
